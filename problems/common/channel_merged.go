...
// by calling `getInputChan` function, 
// it runs a Goroutine which sends number to returned channel
    chanInputNums := getInputChan()

// step 2: `fan-out` square operations to multiple Goroutines
// this can be done by calling `getSquareChan` function 
// multiple times where individual function call returns a channel 
// which sends square of numbers provided by `chanInputNums` channel
// `getSquareChan` function runs Goroutines 
// internally where squaring operation is ran concurrently
